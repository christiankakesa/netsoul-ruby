...
describe Netsoul do
  ...
  it 'Location gives the IONIS internal location or "ext"' do
    expect(Netsoul::Location.get('not exist IP')).to eq('ext')
    expect(Netsoul::Location.get('10.245.42.42')).to eq('etna')
  end
  ...
  describe 'Message handling' do
    it 'returns standard_auth message' do
      ...
      c.build_user_connection_info md5_hash: 'hashx02hash', client_ip: '4.2.4.2', client_port: 4242
    ...
    it 'returns send_message message' do
      message = Netsoul::Message.send_message('user', 'empty message')
      expect(message).not_to be_nil
      expect(message.to_s.size).to be > 0
    end

    it 'returns start_writing_to_user message' do
      start_writing_to_user = Netsoul::Message.start_writing_to_user('user')
    ...
    it 'returns exit command' do
      expect('exit').to eq(Netsoul::Message.ns_exit)
    end
